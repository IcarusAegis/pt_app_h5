<template>
  <main class="v5-screen">
    <!-- 背景装饰层 -->
    <div class="v5-bg-grid"></div>
    <div class="v5-scanlines"></div>
    ...
    <header class="v5-header">
      <div class="v5-brand">
        ...
        <div class="v5-connection" :class="'v5-connection--' + connectionTone">
          <span class="v5-connection-dot"></span>
      ...
      <div class="v5-header-meta">
        <div>
          <span>柜机</span>
          <strong>{{ config.boardId || '--' }}</strong>
        </div>
        <button type="button" class="v5-icon-btn" @click="refreshAll" title="刷新">
          <RefreshCw :size="40" />
        </button>
      </div>
    </header>
    ...
    <section class="v5-workspace">
      <!-- 首页 -->
      <div v-if="viewState === 'home'" class="v5-home">
        <div class="v5-home-heading">
          <p>智能柜机服务系统</p>
          <h1 class="v5-gradient-text">请选择所需服务</h1>
        </div>
        <div class="v5-home-actions">
          <button class="v5-btn-massive v5-btn-take" @click="goToQr('borrow')">
          ...
          <button class="v5-btn-massive v5-btn-store" @click="goToQr('return')">
        ...
        <div class="v5-home-status" :class="'v5-home-status--' + connectionTone">
      ...
      <!-- 二维码页 -->
      <section v-else-if="viewState === 'qr'" class="v5-scan-panel">
        <div class="v5-qr-nav">
          <button class="v5-btn-back" @click="goHome">
            &lt; 返回首页
          </button>
        </div>

        <div class="v5-scan-copy">
          <div>
            <p class="v5-eyebrow">{{ activeCopy.badge }}</p>
            <h2 class="v5-gradient-text">{{ activeCopy.title }}</h2>
            <p class="v5-scan-subtitle">柜体 {{ config.cabinetId || config.boardId || '--' }} · {{ sourceText }}</p>
          </div>
        </div>

        <div class="v5-scan-stage">
          <section class="v5-qr-panel">
            <div class="v5-qr-shell">
              <div class="v5-qr-scanline"></div>
              <canvas ref="qrCanvas" width="500" height="500" aria-label="动态二维码"></canvas>
              <div v-if="qrBusy || cabinetIdentityMissing" class="v5-qr-overlay">{{ qrOverlayText }}</div>
              <div class="v5-qr-corner v5-qr-corner--tl"></div>
              <div class="v5-qr-corner v5-qr-corner--tr"></div>
              <div class="v5-qr-corner v5-qr-corner--bl"></div>
              <div class="v5-qr-corner v5-qr-corner--br"></div>
            </div>
            <div class="v5-qr-meta">
              <div class="v5-timer-ring" :style="{ '--progress': countdownProgress }">
          ...
          <aside class="v5-action-panel">
            <div class="v5-metric-row">
              <div>
                <span>主机编号</span>
                <strong>{{ config.boardId || '--' }}</strong>
              </div>
              ...
              <div>
                <span>服务器连接</span>
                <strong :class="wsConnected ? 'v5-text-cyan' : 'v5-text-red'">{{ wsConnected ? '已连接' : '未连接' }}</strong>
              </div>
            </div>

            <div class="v5-launch-card">
              <div ref="launchHost" class="launch-host"></div>
              <button v-if="!wechatOpenReady" type="button" class="v5-launch-fallback" @click="copyQrContent">
                <Copy :size="40" />
                复制二维码内容
              </button>
            </div>

            <div class="v5-action-note" :class="'v5-action-note--' + connectionTone">
              <span></span>
              <div>
    ...
    <!-- 弹窗 -->
    <div
      class="v5-modal-overlay"
      :class="{ active: animationModalVisible, 'v5-modal-overlay--failed': animationState === 'failed' }"
      @click.self="closeAnimationModal"
    >
      <div class="v5-modal-content">
        <div class="v5-locker-animation">
          <div class="v5-locker-inside"></div>
          <div class="v5-locker-door">
            <span class="v5-locker-number">{{ displayLockId }}</span>
          </div>
          <div class="v5-locker-glow"></div>
        </div>
        <h2 class="v5-modal-title">{{ animationModalTitle }}</h2>
        <p class="v5-modal-desc">
          <template v-if="animationState === 'failed'">
            {{ animationMessage || '请重试或联系管理员' }}
          </template>
          <template v-else>
            请前往 <strong class="v5-highlight-text">{{ displayLockId }}</strong> 号柜存放/提取物品
          </template>
        </p>
        <button type="button" class="v5-modal-close" @click="closeAnimationModal">
