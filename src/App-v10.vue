<template>
  <main class="v10-screen">
    <div class="v10-bg-blob v10-bg-blob--1"></div>
    <div class="v10-bg-blob v10-bg-blob--2"></div>
    <div class="v10-bg-blob v10-bg-blob--3"></div>
    ...
    <header class="v10-header">
      <div class="v10-brand">
        ...
        <div class="v10-connection" :class="'v10-connection--' + connectionTone">
          <span class="v10-connection-dot"></span>
      ...
      <div class="v10-header-meta">
        <div>
          <span>柜机</span>
          <strong>{{ config.boardId || '--' }}</strong>
        </div>
        <button class="v10-icon-btn" @click="refreshAll">
          <RefreshCw :size="36" />
        </button>
      </div>
    </header>

    <section class="v10-workspace">
      <div v-if="viewState === 'home'" class="v10-home">
        <div class="v10-home-heading">
          <p>智能柜机服务系统</p>
          <h1>请选择所需服务</h1>
        </div>
        <div class="v10-home-actions">
          <button class="v10-btn-massive v10-btn-take" @click="goToQr('borrow')">
            <PackageCheck :size="90" />
            <span>
              <small>取件</small>
              扫码开柜取件
            </span>
          </button>
          <button class="v10-btn-massive v10-btn-store" @click="goToQr('return')">
            <ArchiveRestore :size="90" />
            <span>
              <small>存件</small>
              扫码开柜归还
            </span>
        ...
        <div class="v10-home-status" :class="'v10-home-status--' + connectionTone">
          <span class="v10-status-dot"></span>
          <strong>柜机 {{ config.boardId || '--' }}</strong>
          <em>{{ connectionText }}</em>
        </div>
      </div>

      <section v-else-if="viewState === 'qr'" class="v10-scan-panel">
        <div class="v10-qr-nav">
          <button class="v10-btn-back" @click="goHome">
            ← 返回首页
          </button>
        </div>

        <div class="v10-scan-copy">
          <p class="v10-eyebrow">{{ activeCopy.badge }}</p>
          <h2>{{ activeCopy.title }}</h2>
          <p class="v10-scan-subtitle">柜体 {{ config.cabinetId || config.boardId || '--' }} · {{ sourceText }}</p>
        </div>

        <div class="v10-scan-stage">
          <section class="v10-qr-panel">
            <div class="v10-qr-shell">
              <canvas ref="qrCanvas" width="500" height="500"></canvas>
              <div v-if="qrBusy || cabinetIdentityMissing" class="v10-qr-overlay">{{ qrOverlayText }}</div>
            </div>
            <div class="v10-qr-meta">
              <div class="v10-timer-ring" :style="{ '--progress': countdownProgress }">
          ...
          <aside class="v10-action-panel">
            <div class="v10-metric-row">
              <div>
                <span>主机编号</span>
                <strong>{{ config.boardId || '--' }}</strong>
              </div>
              ...
              <div>
                <span>服务器连接</span>
                <strong :class="wsConnected ? 'v10-text-ok' : 'v10-text-err'">{{ wsConnected ? '已连接' : '未连接' }}</strong>
              </div>
            </div>
            <div class="v10-launch-card">
              <div ref="launchHost" class="launch-host"></div>
              <button v-if="!wechatOpenReady" class="v10-launch-fallback" @click="copyQrContent">
                <Copy :size="36" />
                复制二维码内容
              </button>
            </div>
            <div class="v10-action-note" :class="'v10-action-note--' + connectionTone">
              <span></span>
              <div>
    ...
    <div
      class="v10-modal-overlay"
      :class="{ active: animationModalVisible, 'v10-modal-overlay--failed': animationState === 'failed' }"
      @click.self="closeAnimationModal"
    >
      <div class="v10-modal-content">
        <div class="v10-locker-animation">
          <div class="v10-locker-inside"></div>
          <div class="v10-locker-door">
            <span class="v10-locker-number">{{ displayLockId }}</span>
          </div>
        </div>
        <h2 class="v10-modal-title">{{ animationModalTitle }}</h2>
        <p class="v10-modal-desc">
          <template v-if="animationState === 'failed'">
            {{ animationMessage || '请重试或联系管理员' }}
          </template>
          <template v-else>
            请前往 <strong class="v10-highlight-text">{{ displayLockId }}</strong> 号柜存放/提取物品
          </template>
        </p>
        <button class="v10-modal-close" @click="closeAnimationModal">
