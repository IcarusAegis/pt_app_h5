<template>
  <main class="v12-screen">
    <header class="v12-header">
      <div class="v12-brand">
        <div class="v12-logo">KZ</div>
        <div class="v12-connection" :class="'v12-connection--' + connectionTone">
          <span class="v12-connection-dot"></span>
          <span>{{ connectionText }}</span>
        </div>
      </div>
      <div class="v12-header-title">
        <p>科创智借</p>
        <h1>柜屏二维码</h1>
      </div>
      <div class="v12-header-meta">
        <div>
          <span>柜机</span>
          <strong>{{ config.boardId || '--' }}</strong>
        </div>
        <button type="button" class="v12-icon-btn" @click="refreshAll" title="刷新">
          <RefreshCw :size="22" />
        </button>
      </div>
    </header>

    <section class="v12-workspace">
      <section v-if="viewState === 'home'" class="v12-home">
        <div class="v12-home-hero">
          <p>智能柜机服务系统</p>
          <h1>请选择所需服务</h1>
        </div>
        <div class="v12-home-actions">
          <button class="v12-btn-massive v12-btn-take" @click="goToQr('borrow')">
            <div class="v12-btn-icon">
              <PackageCheck :size="36" />
            </div>
            <span>
              <small>扫描二维码开柜取件</small>
              扫码取件
            </span>
          </button>
          <button class="v12-btn-massive v12-btn-store" @click="goToQr('return')">
            <div class="v12-btn-icon">
              <ArchiveRestore :size="36" />
            </div>
            <span>
              <small>扫描二维码开柜归还</small>
              扫码归还
            </span>
          </button>
        </div>
        <div class="v12-home-status" :class="{'v12-home-status--ok': connectionTone === 'ok', 'v12-home-status--warn': connectionTone === 'warn'}">
          <span class="v12-status-dot"></span>
          <strong>柜机 {{ config.boardId || '--' }}</strong>
          <em>{{ connectionText }}</em>
        </div>
      </section>

      <section v-else class="v12-scan-panel">
        <div class="v12-qr-nav">
          <button class="v12-btn-back" @click="goHome">返回首页</button>
        </div>
        <div class="v12-scan-copy">
          <p class="v12-eyebrow">{{ activeCopy.badge }}</p>
          <h2>{{ activeCopy.title }}</h2>
          <p class="v12-scan-subtitle">柜体 {{ config.boardId || '--' }} · {{ sourceText }}</p>
        </div>
        <div class="v12-scan-stage">
          <div class="v12-qr-panel">
            <div class="v12-qr-shell">
              <canvas ref="qrCanvas"></canvas>
              <div v-if="cabinetIdentityMissing || qrBusy" class="v12-qr-overlay">
                {{ qrOverlayText }}
              </div>
            </div>
            <div class="v12-qr-meta">
              <div class="v12-timer-ring" :style="{ '--progress': countdownProgress }">
          ...
          <div class="v12-action-panel">
            <div class="v12-metric-row">
              <div><span>主机编号</span><strong>{{ config.boardId || '--' }}</strong></div>
              <div><span>有效期</span><strong>{{ config.qrTtlSeconds }}s</strong></div>
              <div><span>柜门数</span><strong>{{ doorStats.total || '--' }}</strong></div>
            </div>
            <button class="v12-btn-copy" @click="copyQrContent">
              <Copy :size="16" /> 复制二维码内容
            </button>
            <div class="v12-action-note">
              <p>{{ animationMessage }}</p>
              <small v-if="diagnosticText" class="v12-action-note--detail">{{ diagnosticText }}</small>
            </div>
            <div class="v12-launch-card">
              <div ref="launchHost" class="launch-host"></div>
              <button v-if="!wechatOpenReady" class="v12-launch-fallback" @click="refreshQr">
                刷新小程序码
              </button>
            </div>
          </div>
    ...
    <div class="v12-modal-overlay" :class="{ active: animationModalVisible, 'v12-modal-overlay--failed': animationState === 'failed' }">
      <div class="v12-modal-content">
        <div class="v12-locker-animation">
          <div class="v12-locker-inside"></div>
          <div class="v12-locker-door">
            <span class="v12-locker-number">{{ displayLockId }}</span>
          </div>
        </div>
        <h3 class="v12-modal-title">{{ animationModalTitle }}</h3>
        <p class="v12-modal-desc">
          请前往 <span class="v12-highlight-text">{{ displayLockId }}</span> 号柜存放/提取物品
        </p>
        <button class="v12-modal-close" @click="closeAnimationModal">
          关闭 ({{ closeCountdown }})
        </button>
      </div>
    </div>
