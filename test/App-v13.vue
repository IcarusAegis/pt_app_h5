<template>
  <main class="v13-screen">
    <header class="v13-header">
      <div class="v13-brand">
        <div class="v13-logo">KZ</div>
        <div class="v13-connection" :class="'v13-connection--' + connectionTone">
          <span class="v13-connection-dot"></span>
          <span>{{ connectionText }}</span>
        </div>
      </div>
      <div class="v13-header-title">
        <p>科创智借</p>
        <h1>柜屏二维码</h1>
      </div>
      <div class="v13-header-meta">
        <div>
          <span>柜机</span>
          <strong>{{ config.boardId || '--' }}</strong>
        </div>
        <button type="button" class="v13-icon-btn" @click="refreshAll" title="刷新">
          <RefreshCw :size="22" />
        </button>
      </div>
    </header>

    <section class="v13-workspace">
      <section v-if="viewState === 'home'" class="v13-home">
        <div class="v13-home-hero">
          <p>智能柜机服务系统</p>
          <h1>请选择所需服务</h1>
        </div>
        <div class="v13-home-actions">
          <button class="v13-btn-massive v13-btn-take" @click="goToQr('borrow')">
            <div class="v13-btn-icon">
          ...
          <button class="v13-btn-massive v13-btn-store" @click="goToQr('return')">
            <div class="v13-btn-icon">
        ...
        <div class="v13-home-status" :class="{'v13-home-status--ok': connectionTone === 'ok', 'v13-home-status--warn': connectionTone === 'warn'}">
          <span class="v13-status-dot"></span>
          <strong>柜机 {{ config.boardId || '--' }}</strong>
          <em>{{ connectionText }}</em>
        </div>
      </section>

      <section v-else class="v13-scan-panel">
        <div class="v13-qr-nav">
          <button class="v13-btn-back" @click="goHome">返回首页</button>
        </div>
        <div class="v13-scan-copy">
          <p class="v13-eyebrow">{{ activeCopy.badge }}</p>
          <h2>{{ activeCopy.title }}</h2>
          <p class="v13-scan-subtitle">柜体 {{ config.boardId || '--' }} · {{ sourceText }}</p>
        </div>
        <div class="v13-scan-stage">
          <div class="v13-qr-panel">
            <div class="v13-qr-shell">
              <canvas ref="qrCanvas"></canvas>
              <div v-if="cabinetIdentityMissing || qrBusy" class="v13-qr-overlay">
                {{ qrOverlayText }}
              </div>
            </div>
            <div class="v13-qr-meta">
              <div class="v13-timer-ring" :style="{ '--progress': countdownProgress }">
          ...
          <div class="v13-action-panel">
            <div class="v13-metric-row">
            ...
            <button class="v13-btn-copy" @click="copyQrContent">
              <Copy :size="16" /> 复制二维码内容
            </button>
            <div class="v13-action-note">
              <p>{{ animationMessage }}</p>
              <small v-if="diagnosticText" class="v13-action-note--detail">{{ diagnosticText }}</small>
            </div>
            <div class="v13-launch-card">
              <div ref="launchHost" class="launch-host"></div>
              <button v-if="!wechatOpenReady" class="v13-launch-fallback" @click="refreshQr">
                刷新小程序码
              </button>
            </div>
          </div>
    ...
    <div class="v13-modal-overlay" :class="{ active: animationModalVisible, 'v13-modal-overlay--failed': animationState === 'failed' }">
      <div class="v13-modal-content">
        <div class="v13-locker-animation">
          <div class="v13-locker-inside"></div>
          <div class="v13-locker-door">
            <span class="v13-locker-number">{{ displayLockId }}</span>
          </div>
        </div>
        <h3 class="v13-modal-title">{{ animationModalTitle }}</h3>
        <p class="v13-modal-desc">
          请前往 <span class="v13-highlight-text">{{ displayLockId }}</span> 号柜存放/提取物品
        </p>
        <button class="v13-modal-close" @click="closeAnimationModal">
          关闭 ({{ closeCountdown }})
        </button>
      </div>
    </div>
