<template>
  <main class="v8-screen">
    <div class="v8-ambient v8-ambient--top"></div>
    <div class="v8-ambient v8-ambient--bot"></div>
    ...
    <header class="v8-header">
      <div class="v8-brand">
        ...
        <div class="v8-connection" :class="'v8-connection--' + connectionTone">
          <span class="v8-connection-dot"></span>
          <span>{{ connectionText }}</span>
        </div>
      </div>
      <div class="v8-header-title">
        <p>科创智借</p>
        <h1>柜屏二维码</h1>
      </div>
      <div class="v8-header-meta">
        ...
        <button type="button" class="v8-icon-btn" @click="refreshAll">
          <RefreshCw :size="36" />
        </button>
      </div>
    </header>

    <section class="v8-workspace">
      <div v-if="viewState === 'home'" class="v8-home">
        <div class="v8-home-hero">
          <p class="v8-sup">智能柜机服务系统</p>
          <h1>请选择所需服务</h1>
        </div>
        <div class="v8-home-actions">
          <button class="v8-btn-massive" @click="goToQr('borrow')">
            <div class="v8-btn-icon v8-btn-icon--borrow">
          ...
          <button class="v8-btn-massive v8-btn-massive--store" @click="goToQr('return')">
            <div class="v8-btn-icon v8-btn-icon--store">
        ...
        <div class="v8-home-status" :class="'v8-home-status--' + connectionTone">
          <span class="v8-status-dot"></span>
          <strong>柜机 {{ config.boardId || '--' }}</strong>
          <em>{{ connectionText }}</em>
        </div>
      </div>

      <section v-else-if="viewState === 'qr'" class="v8-scan-panel">
        <div class="v8-qr-nav">
          <button class="v8-btn-back" @click="goHome">
            <span class="v8-back-icon">←</span> 返回首页
          </button>
        </div>

        <div class="v8-scan-copy">
          <p class="v8-eyebrow">{{ activeCopy.badge }}</p>
          <h2>{{ activeCopy.title }}</h2>
          <p class="v8-scan-subtitle">柜体 {{ config.cabinetId || config.boardId || '--' }} · {{ sourceText }}</p>
        </div>

        <div class="v8-scan-stage">
          <section class="v8-qr-panel">
            <div class="v8-qr-shell">
              <canvas ref="qrCanvas" width="500" height="500"></canvas>
              <div v-if="qrBusy || cabinetIdentityMissing" class="v8-qr-overlay">{{ qrOverlayText }}</div>
            </div>
            <div class="v8-qr-meta">
              <div class="v8-timer-ring" :style="{ '--progress': countdownProgress }">
          ...
          <aside class="v8-action-panel">
            <div class="v8-metric-row">
              <div>
                <span>主机编号</span>
                <strong>{{ config.boardId || '--' }}</strong>
              </div>
              ...
              <div>
                <span>服务器连接</span>
                <strong :class="wsConnected ? 'v8-text-ok' : 'v8-text-err'">{{ wsConnected ? '已连接' : '未连接' }}</strong>
              </div>
            </div>
            <div class="v8-launch-card">
              <div ref="launchHost" class="launch-host"></div>
              <button v-if="!wechatOpenReady" class="v8-launch-fallback" @click="copyQrContent">
                <Copy :size="36" />
                复制二维码内容
              </button>
            </div>
            <div class="v8-action-note" :class="'v8-action-note--' + connectionTone">
              <span></span>
              <div>
    ...
    <div
      class="v8-modal-overlay"
      :class="{ active: animationModalVisible, 'v8-modal-overlay--failed': animationState === 'failed' }"
      @click.self="closeAnimationModal"
    >
      <div class="v8-modal-content">
        <div class="v8-locker-animation">
          <div class="v8-locker-inside"></div>
          <div class="v8-locker-door">
            <span class="v8-locker-number">{{ displayLockId }}</span>
          </div>
        </div>
        <h2 class="v8-modal-title">{{ animationModalTitle }}</h2>
        <p class="v8-modal-desc">
          <template v-if="animationState === 'failed'">
            {{ animationMessage || '请重试或联系管理员' }}
          </template>
          <template v-else>
            请前往 <strong class="v8-highlight-text">{{ displayLockId }}</strong> 号柜存放/提取物品
          </template>
        </p>
        <button class="v8-modal-close" @click="closeAnimationModal">
