<template>
  <main class="v11-screen">
    <header class="v11-header">
      <div class="v11-brand">
        <div class="v11-logo">KZ</div>
        <div class="v11-connection" :class="'v11-connection--' + connectionTone">
          <span class="v11-connection-dot"></span>
          <span>{{ connectionText }}</span>
        </div>
      </div>
      <div class="v11-header-title">
        <p>科创智借</p>
        <h1>柜屏二维码</h1>
      </div>
      <div class="v11-header-meta">
        <div>
          <span>柜机</span>
          <strong>{{ config.boardId || '--' }}</strong>
        </div>
        <button type="button" class="v11-icon-btn" @click="refreshAll" title="刷新">
          <RefreshCw :size="22" />
        </button>
      </div>
    </header>
    ...
    <section class="v11-workspace">
      <section v-if="viewState === 'home'" class="v11-home">
        <div class="v11-home-hero">
          <p>智能柜机服务系统</p>
          <h1>请选择所需服务</h1>
        </div>
        <div class="v11-home-actions">
          <button class="v11-btn-massive v11-btn-take" @click="goToQr('borrow')">
            <div class="v11-btn-icon">
          ...
          <button class="v11-btn-massive v11-btn-store" @click="goToQr('return')">
            <div class="v11-btn-icon">
        ...
        <div class="v11-home-status" :class="{'v11-home-status--ok': connectionTone === 'ok', 'v11-home-status--warn': connectionTone === 'warn'}">
          <span class="v11-status-dot"></span>
          <strong>柜机 {{ config.boardId || '--' }}</strong>
          <em>{{ connectionText }}</em>
        </div>
      </section>

      <section v-else class="v11-scan-panel">
        <div class="v11-qr-nav">
          <button class="v11-btn-back" @click="goHome">返回首页</button>
        </div>
        <div class="v11-scan-copy">
          <p class="v11-eyebrow">{{ activeCopy.badge }}</p>
          <h2>{{ activeCopy.title }}</h2>
          <p class="v11-scan-subtitle">柜体 {{ config.boardId || '--' }} · {{ sourceText }}</p>
        </div>
        <div class="v11-scan-stage">
          <div class="v11-qr-panel">
            <div class="v11-qr-shell">
              <canvas ref="qrCanvas"></canvas>
              <div v-if="cabinetIdentityMissing || qrBusy" class="v11-qr-overlay">
                {{ qrOverlayText }}
              </div>
            </div>
            <div class="v11-qr-meta">
              <div class="v11-timer-ring" :style="{ '--progress': countdownProgress }">
          ...
          <div class="v11-action-panel">
            <div class="v11-metric-row">
            ...
            <button class="v11-btn-copy" @click="copyQrContent">
              <Copy :size="16" /> 复制二维码内容
            </button>
            <div class="v11-action-note">
              <p>{{ animationMessage }}</p>
              <small v-if="diagnosticText" class="v11-action-note--detail">{{ diagnosticText }}</small>
            </div>
            <div class="v11-launch-card">
              <div ref="launchHost" class="launch-host"></div>
              <button v-if="!wechatOpenReady" class="v11-launch-fallback" @click="refreshQr">
                刷新小程序码
              </button>
            </div>
          </div>
    ...
    <div class="v11-modal-overlay" :class="{ active: animationModalVisible, 'v11-modal-overlay--failed': animationState === 'failed' }">
      <div class="v11-modal-content">
        <div class="v11-locker-animation">
          <div class="v11-locker-inside"></div>
          <div class="v11-locker-door">
            <span class="v11-locker-number">{{ displayLockId }}</span>
          </div>
        </div>
        <h3 class="v11-modal-title">{{ animationModalTitle }}</h3>
        <p class="v11-modal-desc">
          请前往 <span class="v11-highlight-text">{{ displayLockId }}</span> 号柜存放/提取物品
        </p>
        <button class="v11-modal-close" @click="closeAnimationModal">
          关闭 ({{ closeCountdown }})
        </button>
      </div>
    </div>
