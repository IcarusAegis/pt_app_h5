<template>
  <main class="v7-screen">
    <div class="v7-crt-overlay"></div>
    <div class="v7-scanline"></div>
    ...
    <header class="v7-header">
      <div class="v7-brand">
        ...
        <div class="v7-connection" :class="'v7-connection--' + connectionTone">
          <span class="v7-connection-dot"></span>
          <span>{{ connectionText }}</span>
        </div>
      </div>
      <div class="v7-header-title">
        <p>> SYSTEM: 科创智借</p>
        <h1>柜屏二维码终端</h1>
      </div>
      <div class="v7-header-meta">
        <div>
          <span>CABINET_ID</span>
          <strong>{{ config.boardId || '--' }}</strong>
        </div>
        <button type="button" class="v7-icon-btn" @click="refreshAll" title="刷新">
          <RefreshCw :size="40" />
        </button>
      </div>
    </header>

    <section class="v7-workspace">
      <!-- 首页 -->
      <div v-if="viewState === 'home'" class="v7-home">
        <div class="v7-home-heading">
          <p class="v7-prompt">$ cabinet-service --status</p>
          <h1>> 请选择所需服务_</h1>
        </div>
        <div class="v7-home-actions">
          <button class="v7-btn-massive v7-btn-take" @click="goToQr('borrow')">
          ...
          <button class="v7-btn-massive v7-btn-store" @click="goToQr('return')">
            <ArchiveRestore :size="100" />
            <span>
              <small>[2]</small>
              存件 · RETURN
            </span>
        ...
        <div class="v7-home-status" :class="'v7-home-status--' + connectionTone">
          <span class="v7-status-dot"></span>
          <strong>CABINET_{{ config.boardId || '--' }}</strong>
          <em>{{ connectionText }}</em>
        </div>
      </div>

      <!-- 二维码页 -->
      <section v-else-if="viewState === 'qr'" class="v7-scan-panel">
        <div class="v7-qr-nav">
          <button class="v7-btn-back" @click="goHome">
            &lt; cd /home
          </button>
        </div>

        <div class="v7-scan-copy">
          <div>
            <p class="v7-eyebrow">> STATUS: {{ activeCopy.badge }}</p>
            <h2>{{ activeCopy.title }}</h2>
            <p class="v7-scan-subtitle">CABINET: {{ config.cabinetId || config.boardId || '--' }} | SOURCE: {{ sourceText }}</p>
          </div>
        </div>

        <div class="v7-scan-stage">
          <section class="v7-qr-panel">
            <div class="v7-qr-shell">
              <canvas ref="qrCanvas" width="500" height="500" aria-label="动态二维码"></canvas>
              <div v-if="qrBusy || cabinetIdentityMissing" class="v7-qr-overlay">{{ qrOverlayText }}</div>
            </div>
            <div class="v7-qr-meta">
              <div class="v7-timer-ring" :style="{ '--progress': countdownProgress }">
                <span>{{ secondsLeft }}</span>
              </div>
              <div>
                <span>TIMER_REMAIN</span>
                <strong>UPD: {{ generatedAtText }}</strong>
              </div>
            </div>
          </section>

          <aside class="v7-action-panel">
            <div class="v7-metric-row">
              <div>
                <span>BOARD_ID</span>
                <strong>{{ config.boardId || '--' }}</strong>
              </div>
              ...
              <div>
                <span>CONN</span>
                <strong :class="wsConnected ? 'v7-text-ok' : 'v7-text-err'">{{ wsConnected ? 'ONLINE' : 'OFFLINE' }}</strong>
              </div>
            </div>

            <div class="v7-launch-card">
              <div ref="launchHost" class="launch-host"></div>
              <button v-if="!wechatOpenReady" type="button" class="v7-launch-fallback" @click="copyQrContent">
                <Copy :size="40" />
                $ copy-qr --content
              </button>
            </div>

            <div class="v7-action-note" :class="'v7-action-note--' + connectionTone">
              <span class="v7-note-prefix">></span>
              <div>
    ...
    <!-- 弹窗 -->
    <div
      class="v7-modal-overlay"
      :class="{ active: animationModalVisible, 'v7-modal-overlay--failed': animationState === 'failed' }"
      @click.self="closeAnimationModal"
    >
      <div class="v7-modal-content">
        <div class="v7-locker-animation">
          <div class="v7-locker-inside"></div>
          <div class="v7-locker-door">
            <span class="v7-locker-number">{{ displayLockId }}</span>
          </div>
        </div>
        <h2 class="v7-modal-title">> {{ animationModalTitle }}</h2>
        <p class="v7-modal-desc">
          <template v-if="animationState === 'failed'">
            {{ animationMessage || '请重试或联系管理员' }}
          </template>
          <template v-else>
            请前往 <strong class="v7-highlight-text">LOCKER_{{ displayLockId }}</strong> 存放/提取物品
          </template>
        </p>
        <button type="button" class="v7-modal-close" @click="closeAnimationModal">
          $ exit <span v-if="closeCountdown > 0">({{ closeCountdown }}s)</span>
        </button>
      </div>
    </div>
  </main>
