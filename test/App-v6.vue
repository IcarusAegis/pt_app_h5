<template>
  <main class="v6-screen">
    <!-- 宣纸纹理背景 -->
    <div class="v6-paper"></div>
    ...
    <header class="v6-header">
      <div class="v6-brand">
        ...
        <div class="v6-connection" :class="'v6-connection--' + connectionTone">
          <span class="v6-connection-dot"></span>
      ...
      <div class="v6-header-meta">
        <div>
          <span>柜机编号</span>
          <strong>{{ config.boardId || '--' }}</strong>
        </div>
        <button type="button" class="v6-icon-btn" @click="refreshAll" title="刷新">
          <RefreshCw :size="40" />
        </button>
      </div>
    </header>
    ...
    <section class="v6-workspace">
      <!-- 首页 -->
      <div v-if="viewState === 'home'" class="v6-home">
        <div class="v6-home-heading">
          <p class="v6-poem">物尽其用 · 共享智慧</p>
          <h1>请选择所需服务</h1>
        </div>
        <div class="v6-home-actions">
          <button class="v6-btn-massive v6-btn-take" @click="goToQr('borrow')">
          ...
          <button class="v6-btn-massive v6-btn-store" @click="goToQr('return')">
        ...
        <div class="v6-home-status" :class="'v6-home-status--' + connectionTone">
          <span></span>
      ...
      <!-- 二维码页 -->
      <section v-else-if="viewState === 'qr'" class="v6-scan-panel">
        <div class="v6-qr-nav">
          <button class="v6-btn-back" @click="goHome">
            ← 返回首页
          </button>
        </div>

        <div class="v6-scan-copy">
          <div>
            <p class="v6-eyebrow">{{ activeCopy.badge }}</p>
            <h2>{{ activeCopy.title }}</h2>
            <p class="v6-scan-subtitle">柜体 {{ config.cabinetId || config.boardId || '--' }} · {{ sourceText }}</p>
          </div>
        </div>

        <div class="v6-scan-stage">
          <section class="v6-qr-panel">
            <div class="v6-qr-shell">
              <canvas ref="qrCanvas" width="500" height="500" aria-label="动态二维码"></canvas>
              <div v-if="qrBusy || cabinetIdentityMissing" class="v6-qr-overlay">{{ qrOverlayText }}</div>
            </div>
            <div class="v6-qr-meta">
              <div class="v6-timer-ring" :style="{ '--progress': countdownProgress }">
          ...
          <aside class="v6-action-panel">
            <div class="v6-metric-row">
              <div>
                <span>主机编号</span>
                <strong>{{ config.boardId || '--' }}</strong>
              </div>
              ...
              <div>
                <span>连接状态</span>
                <strong :class="wsConnected ? 'v6-text-ok' : 'v6-text-err'">{{ wsConnected ? '已连接' : '未连接' }}</strong>
              </div>
            </div>

            <div class="v6-launch-card">
              <div ref="launchHost" class="launch-host"></div>
              <button v-if="!wechatOpenReady" type="button" class="v6-launch-fallback" @click="copyQrContent">
                <Copy :size="40" />
                复制二维码内容
              </button>
            </div>

            <div class="v6-action-note" :class="'v6-action-note--' + connectionTone">
              <span></span>
              <div>
    ...
    <!-- 弹窗 -->
    <div
      class="v6-modal-overlay"
      :class="{ active: animationModalVisible, 'v6-modal-overlay--failed': animationState === 'failed' }"
      @click.self="closeAnimationModal"
    >
      <div class="v6-modal-content">
        <div class="v6-locker-animation">
          <div class="v6-locker-inside"></div>
          <div class="v6-locker-door">
            <span class="v6-locker-number">{{ displayLockId }}</span>
          </div>
        </div>
        <h2 class="v6-modal-title">{{ animationModalTitle }}</h2>
        <p class="v6-modal-desc">
          <template v-if="animationState === 'failed'">
            {{ animationMessage || '请重试或联系管理员' }}
          </template>
          <template v-else>
            请前往 <strong class="v6-highlight-text">{{ displayLockId }}</strong> 号柜存放/提取物品
          </template>
        </p>
        <button type="button" class="v6-modal-close" @click="closeAnimationModal">
